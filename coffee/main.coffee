...
setup_chart = (selector, color, data, label_key) ->
  ...
  arcs = vis.selectAll('g.slice').data(pie).enter().append('svg:g').
             attr('class', 'slice').
             attr('title', (d, i) -> format_long_dollars d.data.fy_2014_adopted)
  ...
  ).style('display', (d) -> (if d.visible then null else 'none'))
  ...
  legend = d3.select(selector).append('svg').attr('class', 'legend').
              ...
              selectAll('g').data(color.domain().slice()).enter().append('g').
